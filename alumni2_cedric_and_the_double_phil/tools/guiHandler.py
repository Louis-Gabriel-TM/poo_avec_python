...
class GUIHandler:
    ...
    def logged_menu(self, mode, login):
        ...
        self.insert_btn = self.create_button(self.logged_Window, "Inserer", lambda: [self.logged_Window.destroy(), self.ctl.insert_request(), self.logged_menu(mode, login)], 4, 0)
    ...
    def login_menu(self):
        ...
        self.create_button(self.main_Window, "Connexion", lambda: [self.backup_field(self.login_field, self.password_field), self.main_Window.destroy(), self.ctl.login(self.id, self.password)], 6, 1)
        self.main_Window.mainloop()

    def backup_field(self, login, pwd):
        self.id = login.get()
    ...
    def display_form(self):
        ...
        self.create_button(self.form_Window, "Envoyer", lambda: [self.form_Window.destroy(), self.ctl.submit(), self.login_menu()], 5, 2)
